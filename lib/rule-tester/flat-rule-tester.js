...
const
    ...
    { getRuleOptionsSchema, validateJsonSchemaOnlyAcceptsArraysAtTopLevel } = require("../config/flat-config-helpers"),
...
class FlatRuleTester {
    ...
    run(ruleName, rule, test) {
        ...
        function runRuleForItem(item) {
            ...
            const schema = getRuleOptionsSchema(rule, false);

            if (schema) {
                if (typeof schema !== "object") {

                    // We don't need to cover the array-format case here, it gets normalised to an object
                    throw new Error(`Schema for rule ${ruleName} is invalid: schema must be an object or an array`);
                }
                ...
                if (!validateJsonSchemaOnlyAcceptsArraysAtTopLevel(schema)) {
                    throw new Error(`Schema for rule ${ruleName} is invalid: schema must only accept arrays at the top level`);
                }
            }
