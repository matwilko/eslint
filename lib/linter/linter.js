...
const { canonicalizeEcmaVersion, ecmaVersionToSpecRevision, latestSupportedEcmaVersion } = require("../shared/ecma-versions");
...
function getGlobalsForEcmaVersion(ecmaVersion) {
    return globals[`es${canonicalizeEcmaVersion(ecmaVersion)}`];
}
...
function normalizeEcmaVersion(parser, ecmaVersion) {
    ...
    if (isEspree(parser) && ecmaVersion === "latest") {
        return ecmaVersionToSpecRevision(latestSupportedEcmaVersion);
    }

    return ecmaVersionToSpecRevision(ecmaVersion) || ecmaVersion;
}
...
function normalizeEcmaVersionForLanguageOptions(ecmaVersion) {
    /* If the ecmaVersion is specifically not defined, we default to ES5 */
    ...
    if (typeof ecmaVersion === "number") {
        ...
        /*
         * If the ecmaVersion is a number, we assume it's valid, even
         * though it might well not be.
         */
        return canonicalizeEcmaVersion(ecmaVersion) ||
            (ecmaVersion >= 2015 ? ecmaVersion : ecmaVersion + 2009);
    }

    /*
     * Otherwise, we use the latest supported version.
