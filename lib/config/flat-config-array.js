...
class FlatConfigArray extends ConfigArray {
    ...
    /**
     * Given a resolved config, a type of object to look up,
     * resolves the plugin name and object name for the given
     * object ID, if it exists.
     * @param {Object} config A finalized config object.
     * @param {"rule" | "processor"} objectType The type of object to resolve.
     * @param {string} objectId The ID of the object to resolve.
     * @returns { { pluginName: string, objectName: string } | null}
     * The resolved plugin and object name for the given object ID.
     * If multiple objects resolve to the same ID, the first one found
     * is returned.
     */
    static resolveObjectIdentifier(config, objectType, objectId) {
        if (!config) {
            throw new Error("You must pass a config object.");
        }

        if (objectType !== "rule" && objectType !== "processor") {
            throw new Error("Can only resolve rules or processors.");
        }

        if (typeof objectId !== "string" || !objectId.length) {
            throw new Error("You must pass a non-empty string as the object ID.");
        }

        const { plugins } = config;

        const firstSlash = objectId.indexOf("/");

        // No slash, so we assume a built-in object
        if (firstSlash === -1) {
            if (plugins["@"] && plugins["@"][objectType] && plugins["@"][objectType][objectId]) {
                return { pluginName: "@", objectName: objectId };
            } else {
