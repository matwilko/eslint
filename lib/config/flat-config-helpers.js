...
function validateJsonSchemaOnlyAcceptsArraysAtTopLevel(schema) {
    ...
    if (!compiledSchema.errors) {
        ...
        /*
         * This should never happen but isn't technically impossible
         * (e.g. { "not": { "type": "object" } })
         * If the schema is doing something weird enough to let undefined
         * be a valid value then we should probably ignore it for safety
         */
        return false;
    }
 ...
 * @param {boolean} [validateObjectSchema=false] Whether to apply some validation to an object-format schema
 * @returns {Object} JSON Schema for the rule's options.
 */
function getRuleOptionsSchema(rule, validateObjectSchema = true) {
